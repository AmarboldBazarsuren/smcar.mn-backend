...
exports.getBrands = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: data.map(d => ({ id: d._id, name: d._id, count: d.count })),
    });
  } catch (err) {
...
exports.getModels = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: data.map(d => ({ model: d._id, count: d.count })),
    });
  } catch (err) {
...
exports.getBrandsWithModels = async (req, res) => {
  try {
    ...
    const result = data.map(d => ({
      brand:  d._id,
      total:  d.total,
      models: d.models.sort((a, b) => b.count - a.count).slice(0, 10),
    }));
...
exports.triggerSync = async (req, res) => {
  try {
    ...
    syncService.runSync('manual').catch(err =>
      logger.error(`Manual sync алдаа: ${err.message}`)
    );
